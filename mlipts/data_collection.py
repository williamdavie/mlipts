...
from mlipts.constants import (__diffmethods__, __hpcs__, __MDcodes__,
                              __QMcodes__)
...
class DataCollection:
    ...
    def __init__(self, atom_types: list[str], hpc_config: str = None, **kwargs):
        """
        ...
        self.active_MD_dirs: list[str] = (
            []
        )  # simulation in directory run, active for analysis
    ...
    def build_MD_calculations(
        self,
        MD_base_dir: str,
        variables: dict,
        MDcode: str = "lammps",
        outdir: str = "MD_calculations",
    ) -> None:
        """
        ...
        if MDcode == "lammps":
            new_dirs = lammps.build_lammps_calculations(
                MD_base_dir, variables, outdir=outdir
            )
    ...
    def write_MD_submission_scripts(
        self,
        MD_cmd_line: str,
        ...
        cmd_scipts = write_run_calculation_scripts(
        ...
        )  # leaving as one partition as default for now but more partitions possible easy addition.
        ...
        for i, cmd in enumerate(cmd_scipts):
            ...
            script.write_script(
                f"{scripts_outdir}/MD_submission_script_#{i}", message=False
            )
    ...
    def filter_active_MD(
        ...
        if method == "emd":
            k = input(
                "You opted to filter by earth movers distance, enter number of neighbours (k) to be considered: "
            )
            try:
                k = int(k)
            except:
                raise ValueError("k must be an interger.")
            new_configs, inds = filter.filter_by_emd(
                self.active_MD_configs, tol, k=k, show_dendrograms=show_dendrograms
            )
        elif method not in __diffmethods__:
            raise ValueError(f"Distance metric method {method} not found")

        print(
            f"Filter reduced configuration space from {len(self.active_MD_configs)} to {len(new_configs)}"
        )
    ...
    def build_QM_calculations(
        ...
        label: str = None,
        pre_define_configs: list[ase.Atoms] = None,
    ) -> None:
        """
        ...
        if not self.active_MD_configs and pre_define_configs is None:
            ...
            if not self.active_MD_configs:
                raise ValueError(
                    "MD calculations are active, but no configurations read, you may need to wait for calculations to finish."
                )
        ...
        if pre_define_configs is not None:
            ...
            print(
                f"Number of active configs = number of QM calculation directories = {len(self.active_MD_configs)}"
            )
        ...
        for i, config in enumerate(configs_for_build):
            ...
            if QMcode == "vasp":
                if QM_base_dir is not None:
                    new_calc_dir = vasp.build_vasp_calculation(
                        QM_base_dir, config, f"{label}{extension}", outdir
                    )
                else:
                    base_dir = utils.match_config_to_dir(config, multi_supercell_dict)
                    new_calc_dir = vasp.build_vasp_calculation(
                        base_dir, config, f"{label}{extension}", outdir
                    )
    ...
    def write_QM_submission_scripts(
        self,
        QM_cmd_line: str,
        time_per_partition: str,
        npartitions: int = 1,
        ...
        if smart_convergence == True:
            if equilibrium_config is None:
                raise ValueError(
                    "Cannot perform smart convergence without an expected structure (equilibrium_config)."
                )
            QM_group_indicies, pilot_calculation_configs = group.smart_group_calcs(
            ...
            if pilot_calculations:
                ...
                QM_group_indicies_extended = np.zeros(
                    (QM_group_indicies.shape[0], QM_group_indicies.shape[1] + 1),
                    dtype=np.int16,
                )
                QM_group_indicies_extended[:, 1:] = QM_group_indicies[:, 0:]
                for i in range(npartitions):
                    self.build_QM_calculations(
                        self.initialized_QM_dirs[QM_group_indicies[i, 0]],
                        None,
                        QMcode,
                        calcs_outdir,
                        pre_define_configs=[pilot_calculation_configs[i]],
                        label=f"pilot_#{i}",
                    )
            ...
            self.initialized_QM_dirs = [
                self.initialized_QM_dirs[i]
                for sublist in QM_group_indicies
                for i in sublist
            ]
    ...
    def fetch_MD_configs_from_calcs(self):
        ...
        if not self.active_MD_dirs:
            raise ValueError(
                "Tried to collect configurations from active MD directories but no directories are active."
            )

        for dir in self.active_MD_dirs:
            self.active_MD_configs.extend(
                lammps.read_lammps_output(dir, atom_types=self.atom_types)
            )
    ...
    def set_active_MD_dirs(self, outdir: str = "./MD_calculations") -> None:
        ...
        print(
            f"Number of active MD directories updated from {old_len} to {len(self.active_MD_dirs)}"
        )

        return None

    def set_active_MD_configs(self, config_file: str) -> None:
        """
        ...
        print(
            f"Number of active configs updated from {old_len} to {len(self.active_MD_configs)}"
        )
    ...
    def check_QM_base_dir(self):
        ...
        if self.QM_base_dir is None:
            dir = input(
                "Searched for a Quantum mechanical base directory, none found please define a path: "
            )
    ...
    def set_init_QM_dirs(self, outdir: str = "./QM_calculations") -> None:
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"Tried to read QM calculations in {outdir} but the directory was not found, may need to set param `calc_dirs`"
            )
        subdirs = [p for p in path.iterdir() if p.is_dir()]
        for calc in subdirs:
...
def write_run_calculation_scripts(
    calc_dirs: list[str],
    cmd_line: str,
    npartitions: int = 1,
    ...
    if len(calc_dirs) % npartitions != 0:
        raise ValueError(
            f"Number of calculations to run ({len(calc_dirs)}) is not divisible by number of partitions specified {npartitions}"
        )

    if save_and_remove == True:
        if python_env == None or code == None or database_file == None:
            raise ValueError(
                f"save_and_remove option requires specification of the code used, database to save to and a python enviroment with mlipts."
            )
        if not Path(f"{python_env}/bin/python").exists():
            raise FileNotFoundError(
                f"couldn't find python at ({python_env}/bin/python)"
            )
    ...
    if code == "vasp":
        re_run_cmd = f'if grep -q "aborting loop EDIFF was not reached (unconverged)" OUTCAR; then\necho "Calculation unconverged in $dir — rerunning once"\n{cmd_line}\nfi\n'
    else:
        re_run_cmd = ""
    ...
    for i in range(npartitions):
        ...
        for dir in calc_dirs[
            int(i * num_calcs_per_submission) : int((i + 1) * num_calcs_per_submission)
        ]:
            current_dirs += f"{dir} "
        script += "\n"
