...
class ActiveLearn:
    ...
    def __init__(self, hpc_config: str = None, architecture: str = "mace", **kwargs):
        ...
        if self.architecture == "mace":
            if not self.hpc_config["python_env"]:
                raise ValueError(
                    "To use mace for active learning you must define a python env with mace installed."
                )
        elif self.architecture not in __architectures__:
            raise ValueError(
                f"Active learning for architechture {architecture} not supported, supported architectures: {__architectures__}"
            )
    ...
    def define_commitee(
        self, base_config_file: str, n_models: int, outdir: str = "model_configs"
    ) -> None:
        """
        ...
        self.expected_final_models = [
            f"model_{i}_stagetwo.model" for i in range(n_models)
        ]
    ...
    def train_commitee(
        ...
        num_calcs_per_submission = int(
            len(self.active_model_config_files) / npartitions
        )

        for i in range(npartitions):
            script = hpc_utils.ScriptBuilder(header=header)
            configs_to_run = ""
            for config in self.active_model_config_files[
                int(i * num_calcs_per_submission) : int(
                    (i + 1) * num_calcs_per_submission
                )
            ]:
    ...
    def evaluate_committee(
        ...
        if (self.models_dir is None and models_dir is None) or not Path(
            models_dir
        ).exists():
            raise ValueError(
                "You asked to evaluate a committee models but have not specified where the models are located"
            )
        ...
        if self.architecture == "mace":
            ...
            if self.expected_final_models:
                models = [
                    f"{self.models_dir}/{self.expected_final_models[i]}"
                    for i in range(len(self.expected_final_models))
                ]
            else:
                models = [
                    i
                    for i in all_model_files
                    if ("stagetwo.model" in i and "run" not in i)
                ]
...
class UncertaintyQuantification:
    ...
    def __init__(
        self,
        original_sample_file: str,
        evaluated_config_files: list[str],
        architecture="mace",
    ):
        ...
        self.all_configs = np.empty(
            (self.n_configs, self.committee_size), dtype=ase.Atoms
        )
        ...
        for i in range(self.committee_size):
            ...
            if len(current_configs) != self.all_configs.shape[0]:
                raise ValueError(
                    "Files in the committee have a different number of evaluated configurations"
                )
    ...
    def filter_configs(
        ...
        if max_tol is None:
            ...
            self.indices = np.where(
                (max_tol > self.uncertainties) & (self.uncertainties > tol)
            )[0]
    ...
    def dubois_uncertainty(self) -> tuple:
        ...
        dubois_uncertainties = (
            energy_deviations / all_energy_sd + force_deviations / all_force_sd
        )
    ...
    def energy_standard_deviation(self, configs: list[ase.Atoms]) -> float:
        ...
        for i in range(self.committee_size):
            energies[i] = configs[i].info[self.energy_tag]
    ...
    def ave_force_standard_deviation(self, configs: list[ase.Atoms]) -> float:
        ...
        for i in range(self.committee_size):
            forces[i] = configs[i].arrays[self.forces_tag]
...
def run_active_learn(**kwargs) -> list[ase.Atoms]:
    ...
    print(
        f"Setting up scripts to evaluate models with dependacies: {training_job_id, sampling_job_id}"
    )
