...
def archer2_submission_template(
    nodes: int, ranks: int, time: str, account: str, dependencies: list[str] = None
):
    ...
    if int(minutes) <= 20 and int(hours) == 0 and int(seconds) == 0:
    ...
    if dependencies:
        dependency_str = f"#SBATCH --dependency=afterok:{':'.join(dependencies)}"
    else:
        dependency_str = "\n"
...
def archer2_gpu_submission_template(
    account: str,
    time: str,
    nodes: int = 1,
    gpus: int = 1,
